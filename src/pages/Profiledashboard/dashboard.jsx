  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        console.log('Courses Token:', token); // Debug: Log token
        if (!token) {
        ...
        <>
          ...
          {courses.length > 3 && (
            <div className="mt-6 flex justify-center">
              <button
                onClick={openModal}
                className="px-8 py-3 bg-[#59c1c3] dark:bg-[#3A9D9D] text-white rounded-full font-semibold text-lg hover:bg-[#7ddedf] dark:hover:bg-[#2A8D8D] transition-colors duration-300 shadow-lg"
              >
                View All Courses
              </button>
            </div>
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const token = localStorage.getItem('token');
        console.log('Profile Token:', token); // Debug: Log token
        if (!token) {
